...
let getStore jsonSystem =
    let store =
        ...
        | JsonSystem.Default ->
            DocumentStore.For
                (fun v ->
                    v.Connection(Test.Constants.pgsqlConnectionString "marten.json.default")
                    Marten.NodaTime.NodaTimeExtensions.UseNodaTime(v))
        | JsonSystem.Stj ->
            DocumentStore.For
                (fun v ->
                    v.Connection(Test.Constants.pgsqlConnectionString "marten.json.stj")
        ...
        | JsonSystem.FsStj ->
            DocumentStore.For
                (fun v ->
                    v.Connection(Test.Constants.pgsqlConnectionString "marten.json.fsstj")
                    let serializer = SystemTextJsonSerializer()
        ...
        | JsonSystem.FsStjCustom ->
            DocumentStore.For
                (fun v ->
                    v.Connection(Test.Constants.pgsqlConnectionString "marten.json.fsstj-custom")
                    let serializer = SystemTextJsonSerializer()
...
let ``Record with NodaTime instant`` (jsonSystem: JsonSystem) =
    ...
    task {
        ...
        let! e2 = session2.LoadAsync<RecordWithNodatimeInstant>(e.Id)
        Assert.Equal(e, e2)
    }
